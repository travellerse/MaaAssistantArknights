...
public:
    ...
    void set_runtime(detail::NcnnOcrRuntimeOptions runtime) { m_runtime = runtime; }

    // without_det 时把整张 image 当作一行文本只跑 rec；否则 det -> 裁切 -> rec。
    // 返回的 rect 为 4 点框的轴对齐外接矩形（与 fastdeploy 版语义一致）。
    std::optional<ResultsVec> recognize(const cv::Mat& image, bool without_det);
...
private:
    ...
    std::optional<std::vector<DetBox>> detect(const cv::Mat& image) const;
    // 透视摆正裁出文本行（竖排自动旋正）
    static cv::Mat crop_rotated(const cv::Mat& image, const DetBox& box);
    // 单行文本识别：返回 (utf-8 文本, 平均置信度)
    std::optional<std::pair<std::string, float>> recognize_line(const cv::Mat& line) const;
    ...
    detail::NcnnOcrRuntimeOptions m_runtime;
    int m_cpu_threads = 2;
    bool m_loaded = false;
