...
namespace asst
{
inline constexpr std::pair<BoskyNodeTarget, std::string_view> BoskyNodeTargetNames[] = {
    { BoskyNodeTarget::FindPlaytime, "FindPlaytime" },
    { BoskyNodeTarget::FindSchemeBox, "FindSchemeBox" },
};

std::string_view bosky_node_target_to_string(BoskyNodeTarget target)
{
    for (const auto& [val, name] : BoskyNodeTargetNames) {
        if (val == target) {
            return name;
        }
    }
...
BoskyNodeTarget bosky_node_target_from_string(std::string_view str)
{
    ...
    Log.warn("Unknown BoskyNodeTarget string:", str, ", falling back to FindPlaytime");
    return BoskyNodeTarget::FindPlaytime;
}
} // namespace asst
...
bool asst::RoguelikeConfig::verify_and_load_params(const json::value& params)
{
    ...
    // 设置层数选点策略，相关逻辑在 RoguelikeStrategyChangeTaskPlugin
    {
        ...
        if (m_theme == "JieGarden") {
            ...
            if (m_mode == RoguelikeMode::FindBoskyNode) {
                // 启用刷树洞节点策略，联动 RoguelikeRoutingTaskPlugin
    ...
    if (m_mode == RoguelikeMode::FindBoskyNode) {
        std::string target_str = params.get("find_bosky_node_target", "FindPlaytime");
        m_find_bosky_node_target = bosky_node_target_from_string(target_str);
        Log.info(__FUNCTION__, "| FindBoskyNode target:", target_str);
    }
